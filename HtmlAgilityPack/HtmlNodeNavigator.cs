...
namespace HtmlAgilityPack
{
    ...
    public class HtmlNodeNavigator : XPathNavigator
    {
        ...
        internal HtmlNodeNavigator(HtmlDocument doc, HtmlAttribute  currentAttribute)
        {
            if (currentAttribute == null)
            {
                throw new ArgumentNullException("currentNode");
            }
            if (currentAttribute.OwnerDocument != doc)
            {
                throw new ArgumentException(HtmlDocument.HtmlExceptionRefNotChild);
            }
            InternalTrace(null);
            ...
            _doc = doc;
            Reset();
            _currentnode = currentAttribute.OwnerNode;
            _attindex = _currentnode.Attributes.IndexOf(currentAttribute);
        }
        ...
        /// </summary>
        /// <remarks>In case current position is at attribute, returns parent node. To get attribute use <see cref="Current"/> instead</remarks>
        public HtmlNode CurrentNode
        {
            get { return _currentnode; }
        }

        /// <summary>Gets current node or attribute</summary>
        public IHtmlNode Current
        {
            get
            {
                if (_currentnode == null) return null;
                if (_attindex > -1) return _currentnode.Attributes[_attindex];
                return _currentnode;
            }
        }
        ...
        {
            if (nav == null || !(nav is HtmlNodeNavigator)) return false;
            HtmlNodeNavigator hnav = (HtmlNodeNavigator)nav;
            if (hnav.CurrentDocument != CurrentDocument) return false;
            if (IsSamePosition(hnav)) return false;
            if (hnav._attindex > -1 && hnav.CurrentNode == CurrentNode) return true;
            HtmlNode node = hnav.CurrentNode;
            while (node != null)
            {
                if (node.ParentNode == CurrentNode) return true;
        ...
        public override bool IsSamePosition(XPathNavigator other)
        {
            ...
            return (nav._currentnode == _currentnode && nav._attindex == _attindex);
        }
        ...
        public override bool MoveToFirst()
        {
            ...
            if (_currentnode.ParentNode == null && _currentnode.NodeType == HtmlNodeType.Document)//Root is it's own first sibling
            {
                InternalTrace(">true");
                return true;
            }
            if (_currentnode.ParentNode == null || _currentnode.ParentNode.FirstChild == null)
            {
                InternalTrace(">false");
                return false;
            }
        ...
        {
            if (!_currentnode.HasChildNodes || _attindex > -1)
            {
                InternalTrace(">false");
                return false;
            }
        ...
        {
            if (_currentnode.NextSibling == null || _attindex > -1)
            {
                InternalTrace(">false");
                return false;
            }
        ...
        {
            InternalTrace(null);
            if (_attindex >= (_currentnode.Attributes.Count - 1) || _attindex == -1)
            {
                InternalTrace(">false");
                return false;
            }
        ...
        {
            if (_currentnode.PreviousSibling == null || _attindex > -1)
            {
                InternalTrace(">false");
                return false;
            }
